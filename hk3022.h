...
public:  
  ...
  uint16_t hilimit = 512;        // Верхний предел отключения насоса (верхнее давление системы). По умолчанию 593 (3,0 бар); 512 (2.5 бар)
                                 // (x*818.4/5 + 102.3)
  ...
  uint16_t pumpinittime = 60;    // Время инициализации насоса в секундах (для достижения давления выше предела сухого хода). По умолчанию 40 секунд
  uint16_t pumprunlimit = 40;   // Предел времени непрерывной работы насоса в секундах (исключение длительной работы на сухом ходу). По умолчанию 1 минута
  uint16_t retryinterval = 3600; // Интервал времени для повторения запуска насоса в секундах (при сухом колодце). По умолчанию 20 минут 
  
  hk3022(uint8_t pin, uint8_t pw_pin); // Конструктор без кнопки управления
  void handler(uint32_t unixtime, bool is_timer_synced);     // Обработчик на каждом цикле
