...
void ticker::begin() {
  _t = millis();
  _t5s = _t+4958; // 5000 миллисекунд, скорректировано с учетом рассинхронизации
...
void ticker::handler5s( void (*f)() ) { 
  _t = millis();
  if(_is_overflow && _t&0x80000000) return; // проверка переполнения. ждем пока _t перейдет через 0
  ...
  if(_t >= _t5s ) { 
    ...
    _t5s += 4958; // 5 секунд, скорректировано с учетом рассинхронизации (42 миллмсекунды за 5 секунд)
    if( _t5s < _t ) { 
      // проверка переполнения. 4294967295 произойдет через 49.7 дней после запуска
      _is_overflow = true; // _t5s уже перешел через 0, а _t еще нет
    }
  }
}
